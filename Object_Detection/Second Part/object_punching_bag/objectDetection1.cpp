...
int main(int argc, char* argv[]) {
	...
	while (1) {
		...
		/****************Normalized && mean
		 Scalar tempVal1 = mean(debug_img2);
		 Scalar tempVal2 = mean(template_img);

		 double myMatMean1 = tempVal1.val[0];
		 double myMatMean2 = tempVal2.val[0];
		 Mat debug_res,temp_res;

		 subtract(myMatMean1, debug_img2, debug_res);
		 subtract(myMatMean2, template_img, temp_res);
		 ***************/
		...
		while (i < res.size()) {
			...
			rectangle(frame, matchLoc,
						...
						CV_RGB(0, 0, 255), 3);
			ofile<<"Punching Bag "<<frame_no<<" "<<matchLoc.x<<" "<<matchLoc.y<<" "<<(matchLoc.x + template_img.cols)<<" "<<(matchLoc.y + template_img.rows);
			ofile<<"\n";
		...
		if (minVal < 0.3) {	//0.07 if not zero mean and normalized//normalized-minVal<0.7 && minVal>0.1
